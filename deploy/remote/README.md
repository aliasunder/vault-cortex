...
| `TRUST_FORWARDED_HOPS`   | `1`                           | How many trailing `for=` entries of the `Forwarded` header belong to proxies you control; set `2` when a CDN fronts the proxy that writes it, so the last entry is the CDN, not the visitor |
