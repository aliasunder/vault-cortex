...
| `TRUST_FORWARDED_HOPS`  | `0`                           | How many trailing `for=` entries of the [RFC 7239](https://www.rfc-editor.org/rfc/rfc7239) `Forwarded` header belong to proxies you control; `0` ignores the header. Set `1` only when the proxy in front reports each visitor's IP there (e.g. AWS API Gateway); `2` when a CDN fronts that proxy and is the only way to reach it, so the last entry is the CDN, not the visitor |
