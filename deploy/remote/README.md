...
| `TRUST_FORWARDED_HEADER` | `false`                       | Set `true` only when the proxy in front reports each visitor's IP in the [RFC 7239](https://www.rfc-editor.org/rfc/rfc7239) `Forwarded` header (e.g. AWS API Gateway)                                                       |
| `TRUST_FORWARDED_HOPS`   | `1`                           | How many trailing `for=` entries of the `Forwarded` header belong to proxies you control; set `2` when a CDN fronts the proxy that writes it and is the only way to reach it, so the last entry is the CDN, not the visitor |
