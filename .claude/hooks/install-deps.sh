...
if wait "${install_pid}"; then
  ...
  # SST platform types: build:sst typechecks sst.config.ts via tsconfig.sst.json,
  # which references .sst/platform/config.d.ts — generated by sst install, not npm ci.
  if [[ ! -d "${checkout}/.sst/platform" ]]; then
    log "installing SST platform types"
