...
| `SYNC_FILE_TYPES`                 | _(optional input)_                    | Attachment types to sync: image, audio, video, pdf, unsupported — the same toggles as Obsidian's Sync → Selective sync. Leave empty to sync image, audio, video, and pdf. CSV, JSON, TXT, XML, LOG, and YAML files need unsupported here and **Sync all other types** turned on for the device that uploads them |
