...
| `test_deploy.yml`      | Actions UI (`workflow_dispatch`) | Deploys the selected branch to the instance for testing without cutting a release — builds an amd64-only image pushed as `:test` (never `:latest` or version tags, so released users are unaffected) and points the instance at it via `VAULT_MCP_TAG=test`. Run Force Deploy afterwards to return to the released image. |
