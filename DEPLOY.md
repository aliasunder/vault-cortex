...
> **Upgrading to the release that binds access tokens to `PUBLIC_URL`:**
>
...
> - The Lambda authorizer passes audience-less tokens through this release so that 401 can happen (a Lambda deny is a fixed 403 that MCP clients do not refresh on). The next release makes the Lambda reject them, once every token in flight carries an audience.
