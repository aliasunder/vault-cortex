...
> **Upgrading to the release that binds access tokens to `PUBLIC_URL`:** the Lambda authorizer rejects every access token issued before it, and a Lambda deny is a fixed 403 that MCP clients do not refresh on (they refresh on 401). Each connected client therefore gets 403s until its own token timer runs out (at most 24 hours) and it refreshes silently, or until you reconnect it; refresh tokens are unaffected and nobody re-enters the token. A later change to `PUBLIC_URL` has the same effect.
>
