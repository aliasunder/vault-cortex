...
| `READONLY_MODE`             | Optional. Set `true` to hide every vault-writing tool — read and search only; the memory folder is not auto-created. Default: `false`.                                                                                                                    |
| `DISABLED_TOOLS`            | Optional. Hide individual tools by name, comma-separated. Subtractive only; an unknown tool name stops the server at startup. Default: none hidden.                                                                                                       |
