...
- **`npm run build` fails with `Property 'McpAuthToken' does not exist`** (or `'PublicUrl'`) — `sst-env.d.ts` hasn't been generated. Run `npx sst deploy` (or `sst dev`) once for your stage.
