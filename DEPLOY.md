...
> **Upgrading to the release that binds access tokens to `PUBLIC_URL`:** access tokens issued before it carry no audience. The server rejects them with a 401, and each connected client refreshes silently on its next request; refresh tokens are unaffected and nobody re-enters the token. The Lambda authorizer lets those audience-less tokens through to the server for this release, because a Lambda deny is a fixed 403 that MCP clients do not refresh on; a later release makes the Lambda reject them too, once every token in flight carries an audience. A later change to `PUBLIC_URL` invalidates every access token at the Lambda (403) — reconnect each client, or set the new value only when you can accept that.
>
...
- **Every request gets `403` at the gateway while the instance is healthy** — the Lambda authorizer and Express disagree on `PUBLIC_URL`, so every minted token carries an audience the Lambda rejects, and clients do not recover on their own (they refresh on 401, not 403). The Lambda derives its value at `sst deploy` (the `PUBLIC_URL` env var, else `CUSTOM_DOMAIN`, else the gateway URL); Express reads the instance `.env`. Make them match and redeploy or restart.
