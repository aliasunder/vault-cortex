...
- **Every request gets `401` at the gateway while the instance is healthy** — the Lambda authorizer and Express disagree on `PUBLIC_URL`. The Lambda derives its value at `sst deploy` (the `PUBLIC_URL` env var, else `CUSTOM_DOMAIN`, else the gateway URL); Express reads the instance `.env`. Make them match and redeploy or restart.
