...
| `TRUST_FORWARDED_HOPS`                | Optional. How many entries from the end of the [RFC 7239](https://www.rfc-editor.org/rfc/rfc7239) `Forwarded` header's `for=` list to count to reach the client IP (OAuth rate limiting, request logs); `0` ignores the header. The compose file defaults the instance to `1` (API Gateway reports visitors there and sends no `X-Forwarded-For`). See the **Client-IP trust with ORIGIN_URL** callout for `ORIGIN_URL` and CDN scenarios. |
...
| `DISABLE_EXECUTE_API_ENDPOINT` | Optional, with `CUSTOM_DOMAIN`. Set `true` once every client uses the custom domain: the gateway stops answering on its default `execute-api` hostname at the next deploy. Required before `TRUST_FORWARDED_HOPS=2` when a CDN fronts the custom domain — see the **Client-IP trust with ORIGIN_URL** callout.      |
...
> | Gateway → tunnel/proxy, **unlocked**                                                                         | `2`                | `0`                    | the gateway's address |
> | Gateway → tunnel/proxy, **locked** to the gateway                                                            | `2`                | `1`                    | the visitor           |
...
> - **Other frontends** (Caddy, nginx) can enforce the same lock: `ORIGIN_ACCESS_SERVICE_TOKEN_ENABLED=true` makes the gateway send `CF-Access-Client-Id` / `CF-Access-Client-Secret` on every request, so reject requests without your two values (Caddy: a `header` matcher on `handle`; nginx: an `if` on `$http_cf_access_client_secret`).
