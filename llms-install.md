...
Save, then confirm the connection by calling a read-only tool such as `vault_list_memory_files`. A healthy install discovers **24 tools** (vault CRUD, search, memory, link graph, daily notes) and **3 guided prompts** (`vault-orientation`, `memory-review`, `daily-review`).
