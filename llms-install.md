...
Save, then confirm the connection by calling a read-only tool such as `vault_list_memory_files`. A healthy install discovers tools across multiple categories (vault CRUD, search, tasks, memory, properties, links, and daily notes) and guided prompts (`vault-orientation`, `memory-review`, `daily-review`).
