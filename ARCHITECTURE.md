...
`vault_get_daily_note` resolves the vault's folder and date format per field: `DAILY_NOTES_FOLDER`/`DAILY_NOTES_FORMAT` env overrides → `.obsidian/daily-notes.json` → Obsidian defaults (`Daily Notes/YYYY-MM-DD.md`). Only a successful config-file read is cached; a missing or malformed file falls back uncached and is re-read on the next call, so a config file delivered by initial sync after boot is picked up without a restart (the Tasks plugin format config in `task-format-config.ts` uses the same cache rule). Property tools query the `properties` JSON column in the notes table via `json_each`/`json_extract`, handling both scalar and array-valued properties.
