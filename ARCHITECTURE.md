...
- `outline` returns `{ bytes, modified, leading_callout?, leading_content?, headings }`. Top-level `bytes` is the whole file's on-disk size, while each heading's `bytes` is that section's read cost. If the note changes during the read, the top-level metadata may describe a different file version than the parsed outline.
