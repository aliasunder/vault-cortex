...
- **Chunk prefixes:** every fragment starts with the note title; aggregate and leaf fragments add a `Section:` line naming the heading's ancestor path, while preamble fragments, a singleton wrapper's aggregate, and the TOC chunk keep the bare title. A heading whose slice is empty emits no chunk — its words appear in descendant chunks' Section lines. A top-level heading with children but no body of its own still emits its aggregate (the slice spans the subtree)
