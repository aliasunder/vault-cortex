...
| OAuth 2.1 + static token            | OAuth 2.1 (Authorization Code + PKCE) for browser-capable clients — token refresh is automatic, no secret in config files after initial consent. Static bearer token as a zero-setup alternative for CLI tools, scripts, and MCP Inspector where a browser flow isn't practical. Both validated at two layers (Lambda authorizer + Express middleware) using the same HMAC key, so neither path is a second-class citizen. The static token doesn't expire, but it's also the HMAC signing key — if it leaks, JWTs are compromised anyway, so rotation invalidates everything at once. Single-user threat model: unauthorized access to one person's notes, not lateral movement across accounts.                                                                                                                                                                                    |
