...
- **Chunk prefixes:** note title plus a `Section:` line naming the heading's ancestor path. A heading whose slice is empty emits no chunk — its words appear in descendant chunks' Section lines. A top-level heading with children but no body of its own still emits its aggregate (the slice spans the subtree)
