...
| `vault_create_task` | `path, description, block_id, heading?, parent?, priority?, due?, scheduled?, start?, task_id?, depends_on?, subtasks?, format?`                                      | !destructiveHint |
| `vault_update_task` | `path, block_id?, line?, status?, priority?, description?, due?, scheduled?, start?, created?, task_id?, depends_on?, add_subtask?, assign_block_id?, lane?, format?` | destructiveHint  |
...
`vault_create_task` builds a correctly-formatted task line in one call — description, priority, dates, `block_id`, and optional checklist sub-items — guaranteeing the Tasks-plugin field ordering (description → priority → ➕ created → 🛫 start → ⏳ scheduled → 📅 due → 🆔 task_id → ⛔ depends_on → ^block_id). Cards are always created as `[ ]` (the "creating is not starting" invariant). Placement targets a heading (required on Kanban boards), a parent task (for sub-tasks), or end-of-body. The line builder is a pure string transform in `obsidian-markdown/tasks.ts`; the I/O orchestration lives in `vault-operations/task-updater.ts`.

`vault_update_task` applies status, priority, description, dates, task_id, depends_on, block_id assignment, lane moves, and sub-task additions in a single atomic read-modify-write under one exclusive file lock. All mutations compose in one write cycle. Status changes toggle the checkbox character and manage done/cancelled dates. Description edits replace the text between the checkbox and the first metadata signifier, preserving all fields. Date mutations set or clear the four settable date fields (due, scheduled, start, created) at the correct position per the field ordering convention. Sub-task awareness: when the target is a sub-task (indented line, depth > 0), lane moves are suppressed — the checkbox changes in place. When `status: "done"` is set on a top-level Kanban task without an explicit `lane`, the tool auto-detects the done lane. The `add_subtask` param appends a checklist item under the task, composing with other mutations.

`vault_list_tasks` tracks sub-task depth via an indent stack during extraction: each task carries `depth` (0 for top-level, 1+ for sub-tasks) and `parent_block_id` (the parent's block_id, when available). The `top_level_only` filter excludes sub-tasks from board reads. Depth detection strips blockquote markers before measuring indent and resets at heading boundaries.
