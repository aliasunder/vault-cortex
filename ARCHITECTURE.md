...
4. **PDFs** (`.pdf`) extract text content from the document's content streams (`unpdf`, based on Mozilla's PDF.js). Layout order follows the stream, not visual position, so columns or complex layouts may interleave. Scanned or image-only PDFs with no extractable text return an error stating the file's existence, size, and page count.
