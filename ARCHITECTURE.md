...
**New-directory rescan:** chokidar processes a newly-appeared directory by scanning its contents first and registering the directory's `fs.watch` only after the scan completes, so a file that lands between the scan read and the watch registration is silently lost — the scan didn't see it and no watcher observed the event. The server's atomic write (temp file + rename into a freshly created folder) can hit that window, leaving the note invisible to search until an unrelated later event touches it. The file watcher closes this with a safety net: every `addDir` event schedules a one-shot reconciliation (default 2 × the awaitWriteFinish stability threshold) that lists the directory recursively, compares against chokidar's `getWatched()` tracking, indexes any settled file chokidar missed, and hands every missed entry — files and subdirectories alike — back to `watcher.add()` so future events (unlink included) fire for them. Symlinked directories, which the recursive listing doesn't traverse, get their own reconciliation pass (guarded against symlink cycles by realpath identity) before registration. Files modified within the stability window are not read mid-write: when their directory's watch predates the rescan, the write's own (awaitWriteFinish-gated) event is already in flight and indexes them normally; inside a directory chokidar never watched — where `watcher.add()` registers silently and emits no replay — the reconciliation retries after the write settles.
