...
- **Checklist progress on every entry** — `subtask_progress: { done, total }` aggregates each task's direct children in the same query (a grouped self-join on the parent line), so filtered or `top_level_only` reads still show how far along a card's checklist is. `{ done: 0, total: 0 }` means no checklist; `done` counts status done only, and the counts ignore the query's filters — progress belongs to the card, not the query.
