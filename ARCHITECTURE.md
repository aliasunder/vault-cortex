...
| `daily-review`      | `date?`, `max_chars?` | Daily note content + outgoing links (via `getOutgoingLinks`, with broken-link flags) + backlinks (via `getBacklinks`) + date-specific activity (via `modifiedOnDate`) + structured task data (due/overdue, scheduled, daily-note-scoped via `listTasks`). Guides reconciliation, task review, and pattern recognition. |
