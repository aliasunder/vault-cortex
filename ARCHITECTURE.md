...
**Embedding pipeline:** Controlled by `EMBEDDING_ENABLED` (default: `true`). Notes split into per-heading sections in two views (`chunker.ts`) — each top-level heading spans its full subtree (the aggregate view, so child text embeds twice), while deeper headings own only the lines above the next heading of any level (the disjoint leaf view) — with paragraph sub-splitting for oversized sections (MAX_CHUNK_TOKENS = 450, minus each chunk's prefix cost) and a sub-minimum trailing fragment merged backward. Markdown syntax is stripped before embedding (`plaintext.ts`). Each chunk is prefixed with the note title plus a `Section:` line naming its heading's ancestor path; a heading with no body of its own emits no chunk (its words appear in descendant chunks' Section lines). Each split note with named headings also emits one table-of-contents chunk (the note's folder segments and title on one line, then its heading names in document order, truncated at the chunk budget) — generic intent-phrased queries are structurally won by short chunks under the embedding model, so every split note gets one deliberately short chunk of its own, made unique by its folder path. Short notes (under 500 body tokens) stay a single title-prefixed chunk. Content-hash gating (SHA-256 per chunk) skips re-embedding unchanged content on both incremental file-watcher updates and full rebuilds.
