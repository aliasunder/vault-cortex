...
| `MAX_OAUTH_CLIENTS` | — | `1000` | Maximum number of OAuth clients the server will register. Every MCP client registers itself on first connect; past the cap, new registrations are refused with `registration_limit_reached`. Registrations that never finished consent, or whose tokens have all expired, are removed after a week. Keeps the OAuth database bounded if the `/register` rate limit is ever bypassed. |
