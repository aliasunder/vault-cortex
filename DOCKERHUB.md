...
| `TRUST_FORWARDED_HEADER` | — | `false` | Set `true` only when the proxy in front reports each visitor's IP in the RFC 7239 `Forwarded` header, as AWS API Gateway does — the reference AWS deployment sets this for you. When `false`, that header is ignored. |
