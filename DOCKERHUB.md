...
| `STORAGE_ROOT` | — | — | One directory for everything that must persist — the vault, the search index, and Obsidian Sync state — for hosts that allow a single persistent volume (Railway, Render, Fly.io). Mount the volume there and set this to the same path |
...
| `LOG_DIR` | — | `/data/logs` (remote), unset (local) | Directory for persistent log files. When set, date-stamped log files are written there in addition to the container's own log (`docker logs`), which Docker discards when the container is recreated. Unset or `none` means the container log only. |
