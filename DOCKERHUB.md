...
| `TRUST_FORWARDED_HOPS` | — | `0` | How many entries from the end of the [RFC 7239](https://www.rfc-editor.org/rfc/rfc7239) `Forwarded` header's `for=` list to count to reach the client IP. `0` ignores the header. Set `1` only when the proxy in front reports each visitor's IP there (e.g. AWS API Gateway — the reference AWS deployment sets this for you). That proxy lists its own peer last, so set `2` when a CDN (e.g. Cloudflare) fronts it and is the only way to reach it — the last entry is then the CDN. |
