...
| `MAX_OAUTH_CLIENTS` | — | `1000` | Maximum number of OAuth clients the server will register. Every MCP client registers itself once on first connect; past the cap, new registrations are refused with `registration_limit_reached`. Keeps the OAuth database bounded if the `/register` rate limit is ever bypassed. |
