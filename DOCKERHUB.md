...
| `TRUST_FORWARDED_HOPS` | — | `1` | How many trailing `for=` entries of the `Forwarded` header were written by proxies you control; the client is the entry before them. The proxy that writes the header lists its own peer last, so set `2` when a CDN (e.g. Cloudflare) fronts that proxy and the last entry is the CDN. Only read when `TRUST_FORWARDED_HEADER` is `true`. |
