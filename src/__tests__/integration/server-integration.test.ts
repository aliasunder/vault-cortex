...
const OAUTH_REDIRECT_URI = "http://127.0.0.1/callback"
const OAUTH_TOKEN_A = "integration-token-before-rotation"
const OAUTH_TOKEN_B = "integration-token-after-rotation"
...
describe("rotating MCP_AUTH_TOKEN", () => {
  it("rejects every refresh token issued under the old token and accepts a fresh consent", async () => {
    ...
    const before = await startServer(port, {
      MCP_AUTH_TOKEN: OAUTH_TOKEN_A,
      INDEX_DB_PATH: indexDbPath,
    })
    const client = await registerClient(port)
    const issued = await authorizeOAuth({
      port,
      client,
      authToken: OAUTH_TOKEN_A,
    })
    ...
    const rejected = await refreshToken({
      port: rotatedPort,
      client,
