...
Returns: JSON { path, line, description, block_id, heading, subtasks, changes, advisories, on_completion_applied } — line is the final 1-based position (when on_completion_applied is "delete", it is the position the task occupied before removal); description is the current text; block_id and heading reflect the task after the update (block_id is omitted when the task has none, heading when the task sits above the first heading); subtasks lists each checklist item added by add_subtasks as { line, description } (omitted when none were added) — checklist items carry no block_id, so line is the handle for a follow-up update; changes lists every field applied as "field: before → after", with "(none)" for an absent value (for subtasks the two sides are checklist-item counts); advisories (omitted when the line round-trips clean) lists one sentence per place the stored line parses back differently than this call set — see Obsidian syntax above; on_completion_applied (present only when the task carried 🏁 delete / [onCompletion:: delete] and was transitioned to done) names the action applied — currently "delete".`,
      inputSchema: {
        path: z
          .string()
          .min(1)
