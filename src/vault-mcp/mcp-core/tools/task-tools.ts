...
Obsidian syntax: The Tasks plugin reads metadata off the END of a task line, so a trailing run of signifier syntax inside description or subtasks text — an emoji field like "🔁 every week", or a Dataview [key:: value] field, followed only by other parseable fields — is read back as task metadata rather than text. A signifier followed by ordinary prose stays literal unless the prose matches that field's value grammar — a 🔁 recurrence reads any trailing words as its rule. The same interference can also change what an adjacent metadata field parses back as, or make a field appear that nothing set. The write still succeeds either way; when the stored line would read back differently than submitted, the result carries an advisories array naming each divergence.
...
Obsidian syntax: The Tasks plugin reads metadata off the END of a task line, so a trailing run of signifier syntax inside description or add_subtasks text — an emoji field like "🔁 every week", or a Dataview [key:: value] field, followed only by other parseable fields — is read back as task metadata rather than text. A signifier followed by ordinary prose stays literal unless the prose matches that field's value grammar — a 🔁 recurrence reads any trailing words as its rule. The same interference can also change what an adjacent metadata field parses back as, or make a field appear that nothing set. The write still succeeds either way; when the stored line would read back differently than this call set, the result carries an advisories array naming each divergence. Date stamps a status change writes or clears (the ✅/❌ dates) are expected and never produce advisories.

Returns: JSON { path, line, description, block_id, heading, subtasks, changes, advisories } — line is the final 1-based position; description is the current text; block_id and heading reflect the task after the update (block_id is omitted when the task has none, heading when the task sits above the first heading); subtasks lists each checklist item added by add_subtasks as { line, description } (omitted when none were added) — checklist items carry no block_id, so line is the handle for a follow-up update; changes lists every field applied as "field: before → after", with "(none)" for an absent value (for subtasks the two sides are checklist-item counts); advisories (omitted when the line round-trips clean) lists one sentence per place the stored line parses back differently than this call set — see Obsidian syntax above.`,
      inputSchema: {
        path: z
          .string()
          .min(1)
