...
- At least one change is required. Every field passed is applied in the same single write:
  - status: "todo" | "in_progress" | "done" | "cancelled". Manages checkbox and done/cancelled dates. On a Kanban board, "done" moves the card to the done lane together with its checklist sub-items (their checkboxes are left as they are); a sub-task marked done stays under its parent. Completing a task that carries a 🔁 recurrence rule also writes its next occurrence as a new [ ] task directly above the completed one (below with the plugin's "next line" setting), with dates advanced per the rule and the done lane untouched — exactly as clicking the checkbox in Obsidian does. The new occurrence stays in the source lane and gets no block_id, 🆔, or ⛔. Completing a recurring task by line and retrying after a lost response is NOT idempotent — the spawn occupies the old line, so the retry completes the new occurrence and advances the series twice; prefer block_id (the completed card keeps it).
  - priority: "highest" | "high" | "medium" | "low" | "lowest" sets the signifier; null removes it.
  - recurrence: a Tasks plugin 🔁 rule in natural language ("every week", "every month on the 15th", "every 3 days when done" — "when done" bases the next occurrence on the completion day) sets the rule; null removes it. Passed together with status "done", the new rule governs the spawn (recurrence: null completes without spawning).
...
Returns: JSON { path, line, description, block_id, heading, subtasks, next_occurrence, advisories, changes } — line is the final 1-based position; description is the current text; block_id and heading reflect the task after the update (block_id is omitted when the task has none, heading when the task sits above the first heading); subtasks lists each checklist item added by add_subtasks as { line, description } (omitted when none were added) — checklist items carry no block_id, so line is the handle for a follow-up update; next_occurrence is present only when completing a recurring task spawned one: { line, description, due?, scheduled?, start? } with only the dates the occurrence has — it carries no block_id, so line is its handle; advisories is a list of non-blocking notices about the write (e.g. a recurrence rule that produced no next occurrence), omitted when there are none; changes lists every field applied as "field: before → after", with "(none)" for an absent value (for subtasks the two sides are checklist-item counts, and a spawn adds "next_occurrence: (none) → line N").`,
      inputSchema: {
        path: z
          .string()
          .min(1)
