...
const readJsonBody = async (request: AsyncIterable<Buffer>): Promise<Record<string, unknown>> => {
  const chunks: Buffer[] = []
...
export const startFakeObsidianApi = async (
  ...
  const answer = async (incoming: IncomingMessage, outgoing: ServerResponse): Promise<void> => {
