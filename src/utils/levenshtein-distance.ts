...
export const levenshteinDistance = (first: string, second: string): number => {
  ...
  let previousRow = Array.from({ length: second.length + 1 }, (unusedCell, columnIndex) => columnIndex)
  for (let rowIndex = 1; rowIndex <= first.length; rowIndex++) {
    ...
    for (let columnIndex = 1; columnIndex <= second.length; columnIndex++) {
      ...
      if (deletionBase === undefined || insertionBase === undefined || substitutionBase === undefined) {
      ...
      currentRow.push(Math.min(deletionBase + 1, insertionBase + 1, substitutionBase + substitutionCost))
    }
