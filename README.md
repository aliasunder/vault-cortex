...
| `TRUST_FORWARDED_HEADER`    | —           | `false`                                                                          | Set `true` only when the proxy in front reports each visitor's IP in the [RFC 7239](https://www.rfc-editor.org/rfc/rfc7239) `Forwarded` header (e.g. AWS API Gateway) — the reference AWS deployment sets this for you. When `false`, that header is ignored.                                                                                              |
| `TRUST_FORWARDED_HOPS`      | —           | `1`                                                                              | How many entries from the end of the `Forwarded` header's `for=` list to count to reach the client IP. The proxy that writes the header lists its own peer last, so set `2` when a CDN (e.g. Cloudflare) fronts that proxy, the last entry is the CDN, and the CDN is the only way to reach that proxy. Only read when `TRUST_FORWARDED_HEADER` is `true`. |
