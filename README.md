...
A remote server with a publicly reachable `https` URL adds directly in Claude Desktop's "Add custom connector" dialog — no file editing (like claude.ai, the dialog's connectors are fetched from Anthropic's cloud, not your machine). Only a localhost server needs the file route, because the dialog only accepts `https` URLs: register it in [`claude_desktop_config.json`](https://modelcontextprotocol.io/docs/develop/connect-local-servers) (Claude Desktop → Settings → Developer → Edit Config opens the file) through the [mcp-remote](https://github.com/geelen/mcp-remote) stdio bridge:
