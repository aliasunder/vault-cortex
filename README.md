...
| `PROTECTED_PATHS`           | —           | `MEMORY_DIR`, daily notes folder                                                 | Folders that `vault_delete_note` and `vault_move_note` refuse to touch. Default includes the daily notes folder resolved from `DAILY_NOTES_FOLDER`, `.obsidian/daily-notes.json`, or `Daily Notes`. Overrides the default entirely when set.                                                                                                    |
...
- **Smart defaults** — `MEMORY_DIR` and the daily notes folder feed the defaults for `PROTECTED_PATHS` and `ORPHAN_EXCLUDE_FOLDERS`. Set either only for a fully custom list; an explicit value replaces its default entirely, so include the daily notes folder yourself.
  - `PROTECTED_PATHS` takes the daily notes folder from `DAILY_NOTES_FOLDER`, else `.obsidian/daily-notes.json`, else `Daily Notes`.
