...
| `TRUST_FORWARDED_HEADER`    | —           | `false`                                     | Set `true` only when the proxy in front identifies clients with the RFC 7239 `Forwarded` header (e.g. AWS API Gateway — the reference deployment sets this for you). When `false`, a client-supplied `Forwarded` header is ignored.                                                                       |
