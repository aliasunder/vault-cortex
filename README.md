...
- **Create** — add a correctly-formatted task in one call: description, priority, dates, recurrence, an "On completion" action, block_id, and checklist sub-items, placed under a heading or nested under a parent task
- **Update** — complete, reprioritize, edit the text, set or clear dates and recurrence, add checklist items, and move tasks between headings in a single call. Marking a task done auto-detects the done lane and stamps the completion date; reversing it removes the date. Completing a recurring task creates its next occurrence with dates advanced exactly as the Tasks plugin computes them, and a task carrying an "On completion" delete action is removed instead of left checked off
