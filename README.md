...
A remote server with an `https` URL adds directly in Claude Desktop's "Add custom connector" dialog — no file editing. Only a localhost server needs the file route, because the dialog only accepts `https` URLs: register it in [`claude_desktop_config.json`](https://modelcontextprotocol.io/docs/develop/connect-local-servers) (Claude Desktop → Settings → Developer → Edit Config opens the file) through the [mcp-remote](https://github.com/geelen/mcp-remote) stdio bridge:
...
| `DISABLED_TOOLS`            | —           | —                                                                                | Hide individual tools by name, comma-separated (e.g. `vault_delete_note,vault_move_note`). Names match the Name column in the [tools table](#tools). Subtractive only — it cannot re-enable a tool another setting hides. An unknown tool name stops the server at startup, so typos surface immediately. Cross-references in tool descriptions and prompts adjust to the enabled set automatically; a few durable API-level references (error sections naming sibling tools) remain. |
...
When the file isn't available — or you use the Periodic Notes plugin, whose settings it doesn't reflect — set the values yourself:
