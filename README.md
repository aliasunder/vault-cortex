...
- **Smart defaults** — setting `MEMORY_DIR` or `DAILY_NOTES_FOLDER` automatically updates the defaults for `PROTECTED_PATHS` and `ORPHAN_EXCLUDE_FOLDERS`; when `DAILY_NOTES_FOLDER` is unset, the daily notes folder is resolved from `.obsidian/daily-notes.json`, falling back to `Daily Notes`. Setting `PROTECTED_PATHS` explicitly overrides the default entirely — include the daily notes folder in your list if needed.
