...
- **Smart defaults** — setting `MEMORY_DIR` or `DAILY_NOTES_FOLDER` automatically updates the defaults for `PROTECTED_PATHS` and `ORPHAN_EXCLUDE_FOLDERS`; when `DAILY_NOTES_FOLDER` is unset, `PROTECTED_PATHS` resolves the daily notes folder from `.obsidian/daily-notes.json` (falling back to `Daily Notes`), while `ORPHAN_EXCLUDE_FOLDERS` falls back to `Daily Notes` directly. Setting `PROTECTED_PATHS` explicitly overrides the default entirely — include the daily notes folder in your list if needed.
