...
- **[Tasks](#tools-28)** — Kanban-aware task management: query with structured filters (status, dates, priority, folder, heading), then complete, reprioritize, or move tasks between lanes — all in one atomic call that auto-detects the done lane. Parses both [Tasks plugin](https://publish.obsidian.md/tasks/) emoji and [Dataview](https://blacksmithgu.github.io/obsidian-dataview/) inline-field formats.
