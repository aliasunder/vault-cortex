...
# Set TRUST_FORWARDED_HOPS=1 only if the proxy reports each visitor's IP in
# the RFC 7239 Forwarded header (https://www.rfc-editor.org/rfc/rfc7239,
# e.g. AWS API Gateway); 0 ignores that header. That proxy lists its own
# peer last; with a CDN (e.g. Cloudflare) in front of it the last entry is
# the CDN, so set TRUST_FORWARDED_HOPS=2 to read the entry before it — only
# when the CDN is the sole way to reach that proxy.
# TRUST_PROXY_HOPS=0
