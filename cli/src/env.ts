...
#
#   What sits in front of the server       TRUST_PROXY_HOPS  TRUST_FORWARDED_HOPS
#   Nothing (default)                      0                 0
#   A proxy or tunnel you control          1                 0
#     (Caddy, nginx, Cloudflare Tunnel)
#   A proxy that reports visitors in       1                 1
#     Forwarded (e.g. AWS API Gateway)
#   A CDN in front of that proxy, and      as above          2
#     the CDN is the only way to reach it
#
# Faked headers from visitors are ignored in every row — only the counted
# proxies are trusted. The CDN row adds to whichever row above describes
# the proxy behind the CDN.
# TRUST_PROXY_HOPS=0
