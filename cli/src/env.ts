...
# Client IP for OAuth rate limiting and logs (defaults shown). Without the
# right values, every visitor behind a proxy shares one rate-limit budget.
# TRUST_PROXY_HOPS counts proxies you control that write X-Forwarded-For;
# TRUST_FORWARDED_HOPS counts ones that write the RFC 7239 Forwarded header
# (https://www.rfc-editor.org/rfc/rfc7239) — 0 ignores that header.
#
#   What sits in front of the server                PROXY_HOPS  FORWARDED_HOPS
#   Nothing (default)                               0           0
#   A proxy or tunnel you control (Caddy, nginx,    1           0
#     Cloudflare Tunnel)
#   A proxy that reports visitors in Forwarded      1           1
#     (e.g. AWS API Gateway)
#   A CDN in front of that proxy, and the CDN is    same        2
#     the only way to reach it
#
# Faked headers from visitors are ignored at every row — only the counted
# proxies are trusted.
# TRUST_PROXY_HOPS=0
