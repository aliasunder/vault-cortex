...
# TRUST_FORWARDED_HOPS controls the RFC 7239 Forwarded header
# (https://www.rfc-editor.org/rfc/rfc7239): 0 ignores it; 1 reads the
# last for= entry as the client IP. Set 1 only when the proxy reports
# each visitor's IP there (e.g. AWS API Gateway).
#
# With a CDN (e.g. Cloudflare) in front of that proxy, the last entry is
# the CDN. Set TRUST_FORWARDED_HOPS=2 to read the entry before it — only
# when the CDN is the sole way to reach the proxy.
# TRUST_PROXY_HOPS=0
