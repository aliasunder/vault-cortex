...
# YYYY-MM-DD) — read directly from your vault folder, no extra setup needed.
# Set to override per field — useful when the config file is absent or you
# use the Periodic Notes plugin. Format takes the same tokens as Obsidian's
# daily note date format setting.
# DAILY_NOTES_FOLDER=Journal
# DAILY_NOTES_FORMAT=YYYY-MM-DD

# Comma-separated folders protected from deletion (default: MEMORY_DIR plus
# your daily notes folder — DAILY_NOTES_FOLDER when set, otherwise "Daily Notes").
# A custom folder set only in daily-notes.json is not auto-protected — override
# to include it.
# PROTECTED_PATHS=About Me,Daily Notes

# Comma-separated folders excluded from orphan detection
# (default: daily notes folder, Templates, MEMORY_DIR).
...
# Daily notes folder and filename format. When unset, both are read from the
# vault's .obsidian/daily-notes.json (falling back to "Daily Notes" and
# YYYY-MM-DD) — the file reaches the server via SYNC_CONFIGS (below). Set to
