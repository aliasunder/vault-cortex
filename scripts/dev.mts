...
const waitForDocker = (ip: string, id: string, timeoutSec = 120): void => {
  ...
  console.error(`✕ Docker not available on ${ip} after ${timeoutSec}s. Check cloud-init logs.`);
  process.exit(1);
};
...
switch (sub) {
  ...
  case "lightsail:up": {
    ...
    run(`ssh ${id} ${sshOpts} ubuntu@${ip} 'sudo mkdir -p /opt/vault-cortex && sudo chown ubuntu:ubuntu /opt/vault-cortex'`);
    waitForDocker(ip, id);
    const ghcrToken = env.GHCR_TOKEN;
    if (!ghcrToken) {
      console.error(`✕  GHCR_TOKEN not set in ${ENV_PATH}. Needed for docker login on the instance.`);
      process.exit(1);
    }
